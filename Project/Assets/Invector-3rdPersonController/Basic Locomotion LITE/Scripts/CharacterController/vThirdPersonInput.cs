...
namespace Invector.CharacterController
{
    public class vThirdPersonInput : MonoBehaviour
    {
        ...
        {
            bool isSprinting = Input.GetKey(sprintInput);

            cc.Sprint(isSprinting);
            float fov = GameRegimeManager.Instance.personCamera._camera.fieldOfView;
            float targetFov = isSprinting ? 90 : 60;
            float factor = 1 - (targetFov - fov) / targetFov;
            GameRegimeManager.Instance.personController.freeSprintSpeed = 4 +
                (isSprinting ? factor * 2 : 0);
            fov = Mathf.Lerp(fov, targetFov, 0.05f);
            GameRegimeManager.Instance.personCamera._camera.fieldOfView = fov;
        }
